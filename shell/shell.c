...
void print_prompt(void) {
   printf("[KimWeng@localhost %s]$ ", cwd_cache);
}
...
/*~~~~~以下code只是測試cmd_parse函數和make_clear_abs_path函數有沒有正常運作而已，在第15章f以後棄用~~~~~
	  char buf[MAX_PATH_LEN] = {0};
      int32_t arg_idx = 0;
      while(arg_idx < argc) {
		//printf("%s ", argv[arg_idx]); <==第15章e以前採用此
...
/*	  ##把argv內全部清回NULL，應該不需要，
	  ##因為運行下一個新的指令時，argv內的內容會被覆蓋掉。	*/
	  int32_t arg_idx = 0;
      while(arg_idx < MAX_ARG_NR) {
		argv[arg_idx] = NULL;
		arg_idx++;
