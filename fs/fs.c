...
/**==>	##1個文件(檔案)占用1個inode，1個inode對應點陣圖的1位元，最多支持4096個文件，所以要用4096個位元(MAX_FILES_PER_PART)的點陣圖來表示，
		##DIV_ROUND_UP(MAX_FILES_PER_PART, BITS_PER_SECTOR)為inode用的 4096個位元(MAX_FILES_PER_PART)的 點陣圖 所占用的扇區數，
		##一個扇區數的位元數為4096，所以4096/4096=1。		**/
	
    uint32_t inode_table_sects = DIV_ROUND_UP(((sizeof(struct inode) * MAX_FILES_PER_PART)), SECTOR_SIZE);
/**==>	##1個文件要用1個inode，所以4096個文件占用的位元組數為sizeof(struct inode) * MAX_FILES_PER_PART
		##DIV_ROUND_UP(((sizeof(struct inode) * MAX_FILES_PER_PART)), SECTOR_SIZE)為inode占用的扇區數。	**/	

    uint32_t used_sects = boot_sector_sects + super_block_sects + inode_bitmap_sects + inode_table_sects;
/**==>	##上面四個已經使用了used_sects個扇區。	**/

    uint32_t free_sects = part->sec_cnt - used_sects;
/**==>	##還剩下part->sec_cnt - used_sects個扇區能用。	**/
	

    /**========================================= 简单处理块位图占据的扇区数 =========================================**/
    uint32_t block_bitmap_sects;
    block_bitmap_sects = DIV_ROUND_UP(free_sects, BITS_PER_SECTOR);
/**==>	##剩下的扇區如果一個扇區用點陣圖的1位元表示，那這點陣圖占用的扇區數為DIV_ROUND_UP(free_sects, BITS_PER_SECTOR)，
		##這點陣圖叫做"塊點陣圖"。	**/

    /* block_bitmap_bit_len是位图中位的长度,也是可用块的数量 */
    uint32_t block_bitmap_bit_len = free_sects - block_bitmap_sects;
/**==>	##把剩下能用的扇區減掉塊點陣圖所用的扇區數為剩下的"真"空閒扇區數。	**/

    block_bitmap_sects = DIV_ROUND_UP(block_bitmap_bit_len, BITS_PER_SECTOR); 
/**==>	##如果 "真"空閒扇區數的 一個扇區用點陣圖的1位元表示，
		##那DIV_ROUND_UP(block_bitmap_bit_len, BITS_PER_SECTOR)表示"真"空閒扇區數所用的點陣圖 占用的扇區數大小，
		##把這大小更新到block_bitmap_sects中。	**/
    /**==============================================================================================================**/
    ...
    sb.root_inode_no = 0; //##根目錄的inode_no為0
        ...
        while(dev_no < 2) {
            ...
            while(part_idx < 12) {   	// 4个主分区+8个逻辑
                ...
                if (part->sec_cnt != 0) {  // 如果分区存在
                    ...
                    if (sb_buf->magic == 0x19970912) {
                        printk("%s has filesystem\n", part->name); //##part->name為該分區(part)的名字
