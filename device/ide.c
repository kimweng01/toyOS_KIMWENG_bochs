...
void ide_init() {
   ...
   直到硬盘完成后通过发中断,由中断处理程序将此信号量sema_up,唤醒线程. */
      ...
      while (dev_no < 2) {
		...
		hd->my_channel = channel;	//(&channel->devices[dev_no])->my_channel = channel;
		hd->dev_no = dev_no;		//(&channel->devices[dev_no])->dev_no = dev_no;
...
struct ide_channel {  <--------------------------------------------------------
   ...
   struct lock lock;                                                          |
   bool expecting_intr;		                                                  |
   struct semaphore disk_done;           硬盘结构                      		  |
   struct disk devices[2];	-----------> struct disk {   <---------------------------------------------------------------
};                                          char name[8];			   		  |                                         |
                                            struct ide_channel* my_channel;	---                                         |
                                            uint8_t dev_no;			   		             分区结构                       |
                                            struct partition prim_parts[4];	 ----------> struct partition {             |
                                            struct partition logic_parts[8];                uint32_t start_lba;		 	|
                                                                                            ...
                                                                                            struct bitmap block_bitmap; -------------------------------------> struct bitmap {                   uint32_t magic;		    
                                                                                            struct bitmap inode_bitmap; -----> struct bitmap {                    uint32_t btmp_bytes_len        uint32_t sec_cnt;		    
                                                                                            struct list open_inodes;	          uint32_t btmp_bytes_len         uint8_t* bits;                 uint32_t inode_cnt;		 
                                                                                         };                                       uint8_t* bits;               };                                uint32_t part_lba_base;	 
                                                                                                                               };                                                                  
...
引导扇区,mbr或ebr所在的扇区                                                                                                                                                                   
struct boot_sector {                                                                                                                                                                             uint32_t inode_table_lba;	
   uint8_t  other[446];                                         构建一个16字节大小的结构体,用来存分区表项                                                                           uint32_t inode_table_sects;
   struct   partition_table_entry partition_table[4]; --------> struct partition_table_entry {                                                                         
   uint16_t signature;                                             uint8_t  bootable;		 // 是否可引导	                                                                            uint32_t data_start_lba;	 
} __attribute__ ((packed));                                        uint8_t  start_head;		 // 起始磁头号                                                                            uint32_t root_inode_no;	 
                                                                   uint8_t  start_sec;		 // 起始扇区号                                                                            uint32_t dir_entry_size;	 
                                                                   uint8_t  start_chs;		 // 起始柱面号                                                                         
                                                                   uint8_t  fs_type;		 // 分区类型                                                                            uint8_t  pad[460];		    
                                                                   uint8_t  end_head;		 // 结束磁头号                                                                         } __attribute__ ((packed));
                                                                   uint8_t  end_sec;		 // 结束扇区号
                                                                   uint8_t  end_chs;		 // 结束柱面号
