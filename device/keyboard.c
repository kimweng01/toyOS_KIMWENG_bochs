...
/* 如果上次是以0xe0开头,将扫描码合并 */
   ...
   //###擷取scanecode的第8位，若為1，表示現在收到的是斷碼。
...
/*	###不管現在收到的是通碼還是斷碼，一律把此code改成通碼型式賦予make_code，讓其送進if去判斷，
	###方法為把scancode的第8位元抹掉。	*/
...
/*	###ctrl、shift、alt為按住不放(收到通碼，未收到斷碼)有效，放開無效(收到斷碼後)，
	###caps_lock為按下後狀態反轉，放開後無任何影響，
	###所以在if (break_code)中不理會caps_lock，即沒有caps_lock的程式，
	###
	###只有ctrl、shift、alt的斷碼會對輸入結果有影響，
	###所以if (break_code)只處理ctrl、shift、al，
	###不理會英文、數字、標點符號、caps_lock等其他輸入!	*/
...
/*	###目前設計僅支援的最大通碼為0x3a，
	###所以用 scancode > 0x00 && scancode < 0x3b 來約束，
	###但alt_r_make和ctrl_r_make都在範圍之外，
	###所以特別把alt_r_make和ctrl_r_make也OR進去，
	###讓中斷處理常式也可以支援。	*/
      ...
      if ((scancode < 0x0e) || (scancode == 0x29) || \
	    ...
	    *******************************/
		...
		else { // 如果shift和capslock都沒被按下
			shift = false;
		}
      }

      uint8_t index = (scancode &= 0x00ff);  // 将扫描码的高字节置0,主要是针对高字节是e0的扫描码.
/*	###高位元組e0只是讓中斷處理常式知道接下來還有輸入碼用，
	###所以先執行返回，再讀下一個輸入碼，
	###程式運行到這裡時高位元組e0已經沒用處了，且index只認低位元組，
	###所以在此將有e0的輸入碼的e0部分抹掉。	*/
